...
|---|---|
| **Admin** | Tudo do Atendente + cadastrar/editar vendedores, **resetar senhas** (as duas coisas na tela **Equipe**, §17), ver a tela de **Erros**, **limpar** o log de erros |
...
- **Resetar a senha NÃO derruba a sessão já aberta.** Medido (ago/2026): depois de o admin resetar a senha, o celular que estava logado **continuou funcionando**. O reset vale do **próximo login** em diante — ele serve para quem esqueceu a senha, não para expulsar.
- **Desativar (`ativo = false`) corta ESCRITA e LEITURA.** A escrita desde a `0025`; a leitura só desde a **`0052`** (issue #91) — antes disso a `0025` cobria apenas a escrita (*"um desativado ainda loga e VÊ"*), e o barrar na entrada era só do frontend ([Painel.jsx:203](src/components/Painel.jsx:203)), que não protege quem usa a API direto. A `0052` fecha isso com **policies `restrictive`** nas 8 tabelas de conteúdo — restritiva o Postgres soma com **E** a tudo que já existe, então nenhuma condição antiga precisou ser reescrita (reescrever oito à mão é como um pedaço de condição some sem ninguém ver).
  > A **`perfil` ficou de fora de propósito**: sem ler o próprio perfil, o Painel receberia erro e o aviso *"Sua conta está desativada"* viraria *"Seu usuário ainda não tem um perfil cadastrado"* — a mensagem errada para quem foi desativado por engano.
  > **A sessão em si não cai**: o token continua válido. O desativado simplesmente não vê mais nada. Encerrar sessões de verdade continua sendo no painel do Supabase.
...
6. ~~Reset de senha in-app~~ → **feito** (ago/2026): bloco **"Senha"** dentro do "Editar membro", na Equipe. Quem esquece a senha não entra e, por não entrar, não alcança o "Meu perfil" para trocá-la — o admin define uma nova pela Edge Function **`resetar-senha`**. O admin **não reseta a si mesmo** por ali (usa o Meu perfil, que pede a senha duas vezes). O "esqueci minha senha" por email foi **descartado**: o §15 põe email fora de escopo, exigiria SMTP configurado e o link de recuperação volta com um token na URL, que este app não lê (navegação por estado, sem router).
   > As duas funções de admin (`criar-usuario` e `resetar-senha`) compartilham o portão em **`supabase/functions/_shared/admin.ts`** — elas rodam com `service_role`, **por fora da RLS**, e a checagem de "admin ativo" já foi esquecida uma vez. Mexer no portão obriga a **redeployar as duas**.
7. ~~Export/backup dos dados~~ → **feito**: Administração → **Backup** baixa um `.zip` com as 9 tabelas de conteúdo em **CSV** (separador `;`, o do Excel em português) e **JSON**, mais um `LEIA-ME`. Os **arquivos** dos anexos têm botões próprios, um por tipo (saída ~60 MB / entrada ~45 MB), organizados numa pasta por demanda — separados porque o zip é montado na memória do navegador, e por isso o botão fica **desabilitado no celular**. Sem dependência (reusa o `lib/zip.js` da #72) e sem migração. Não substitui o backup automático do Supabase; resolve outra coisa — ter os dados fora dele, legíveis.
