...
- **Nem resetar a senha nem desativar derrubam uma sessão JÁ ABERTA.** Medido (ago/2026): depois de o admin resetar a senha, o celular que estava logado **continuou funcionando**. E a `0025` diz no próprio cabeçalho que ela cobre só a **escrita** — *"um desativado ainda loga e VÊ, mas não FAZ mais nada"*; o barrar na entrada é do frontend ([Painel.jsx:203](src/components/Painel.jsx:203)), que só roda quando a tela monta. Ou seja: as duas ações valem do **próximo login** em diante. **Tirar o acesso de alguém AGORA é no painel do Supabase** (revogar as sessões). O reset serve para quem esqueceu a senha, não para expulsar.
