...
## 1. ✅ Migrações: todas aplicadas (`0001` → `0048`) — a `0049` fica para DEPOIS do deploy
...
## 8. 🎯 Próximo passo
**Rodar a `0049`** (o `check`) assim que o deploy da fase 3 estiver no ar e testado no app — ela é o que leva a exigência para o banco. Depois disso, a `#85` fecha. Fica para uma migração futura remover a coluna `obra.endereco` (legado sem uso). Antes: obra obrigatória em todos os tipos + `cidade_estado` obrigatório (migração `0047`, obra antiga sem o campo pede na hora e grava na obra). Backlog: export/backup, #29 (reatribuir demandas existentes), #43/#32/#18/#17/#16.
