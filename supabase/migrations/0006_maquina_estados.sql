-- ───────────────────────────────────────────────────────────────
-- Migracao 0006 — Fase 3c: maquina de estados
--
-- Cria:
--   - historico_status (log das transicoes) + RLS de leitura
--   - mover_status(): a UNICA porta para mudar o status. Concentra as
--     regras dos §7 (transicoes) e §13 (comentario obrigatorio), de
--     forma atomica. A tabela demanda segue SEM policy de update, entao
--     o status so muda por esta funcao.
--
-- Como aplicar: cole este arquivo inteiro no SQL Editor do Supabase e rode.
-- ───────────────────────────────────────────────────────────────

-- ① historico_status — o log de cada transicao ───────────────────
create table historico_status (
  id            bigint         generated always as identity primary key,
  demanda_id    bigint         not null references demanda (id)    on delete cascade,
  de_status     status_demanda not null,
  para_status   status_demanda not null,
  autor_id      uuid           not null references perfil (id)     on delete restrict,
  comentario_id bigint         references comentario (id)          on delete set null,
  created_at    timestamptz    not null default now()
);

alter table historico_status enable row level security;

-- Leitura: enxerga o historico quem enxerga a demanda-pai.
create policy "historico_leitura" on historico_status
  for select
  using (
    exists (
...
-- Sem insert/update/delete: apenas a funcao mover_status escreve aqui.

-- ② mover_status() — a "porteira" das transicoes ─────────────────
...
begin
  ...
  -- 7) Move o status.
  update demanda set status = p_novo_status where id = p_demanda_id;
...
-- Usuarios logados podem chamar a funcao (ela mesma barra o vendedor).
grant execute on function public.mover_status(bigint, status_demanda, text) to authenticated;
